...
- **Data quality:** `data_quality.py` gates every hospital build, on WRITE-AUDIT-PUBLISH -- the snapshot lands in `stg_hospital_profile`, the gates run on staging, and only then does it replace `gold_hospital_profile`. Never build straight into the published table: the gates become decorative because CREATE OR REPLACE has already swapped bad data in. ERROR (blocks the build): duplicate business key, fan-out, empty table, a mapped column that is ENTIRELY NULL, values outside the state filter. WARN: coverage below floor, row-count drift. `--allow-quality-failures` overrides. NOTE: `ed_volume` is TEXT (`very high`/`high`/`medium`/`low`), NOT a number -- it was a DOUBLE emptied by TRY_CAST for the life of the project, and the 35-case eval passed anyway because ground truth is reference_sql over the same empty column. An eval suite validates the AGENT, not the DATA.
...
4. CI runs three workflows: `tests.yml` (pytest on push/PR — the **full** unit/integration/security suite, 481 passing, **no deselects**; only the opt-in `network` marker is skipped), `eval-on-push.yml` (6-case hospital gate), and `eval-nightly.yml` (two jobs: 38-case hospital + 28-case FHIR, 3 runs each, the latter building its Gold from Synthea with a cache). A task is not complete if any of these would fail. Evals run on **Gemini** (`gemini-flash-lite-latest`, free tier, `PLANNER_MIN_INTERVAL_SEC=5`). Last full single-provider sweep: **63/63 cases at 100%** (35/35 hospital + 28/28 FHIR) on the PREVIOUS hospital Gold. The hospital suite is now 38 cases on a corrected Gold and all 38 pass at HEAD, but split across two providers (Gemini quota exhausted 19 cases in, remainder on Groq: 38/38 cases, 111/114 runs, 3 guard denials on Groq only). A single-provider 38-case sweep is owed once the daily quota resets -- do not quote a 38/38-on-Gemini number until one exists. Groq's free tier 429s under full load; groq/cerebras/xai/anthropic remain selectable.
